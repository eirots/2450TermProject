...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->dataGridMemory->ColumnHeadersDefaultCellStyle = dataGridViewCellStyle1;
			...
			this->dataGridMemory->DefaultCellStyle = dataGridViewCellStyle2;
			...
			this->dataGridMemory->RowHeadersDefaultCellStyle = dataGridViewCellStyle3;
			this->dataGridMemory->RowHeadersVisible = false;
			dataGridViewCellStyle4->Alignment = System::Windows::Forms::DataGridViewContentAlignment::MiddleCenter;
			this->dataGridMemory->RowsDefaultCellStyle = dataGridViewCellStyle4;
			...
			this->toolTip1->SetToolTip(this->buttonOpen, L"Select to add value in Input Value to the memory address noted in the console.");
			...
			this->toolTip1->SetToolTip(this->buttonLoad, L"Select to add value in Input Value to the memory address noted in the console.");
			...
			this->toolTip1->SetToolTip(this->buttonEdit, L"Select to add value in Input Value to the memory address noted in the console.");
			...
			this->toolTip1->SetToolTip(this->buttonSave, L"Select to add value in Input Value to the memory address noted in the console.");
...
#pragma endregion
		...
		void PopulateMemoryGrid()
		{
			...
			for (size_t i = 0; i < memory.size(); ++i) {
				...
				dataGridMemory->Rows->Add(address, marshal_as<String^>(memory[i]));
			}
		}
	private:
		System::Void buttonAdd_Click(System::Object^ sender, System::EventArgs^ e) {
			if (ValidateInputCommand(inputCommand->Text)){
				...
				std::string commandVal = marshal_as<std::string>(inputCommand->Text);
		...
		System::Void buttonRun_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			do{
				...
				int instruction;
				if (Int32::TryParse(marshal_as<String^>(uvsimulator->getMemory(pc)), instruction)) {
					...
					{
						UpdatePCLabel();
						String^ word = marshal_as<String^>(uvsimulator->getMemory(operand));
						if (screen->Text == "") {
							screen->Text += word;
						}
						else {
							screen->Text += Environment::NewLine + word;
						}
						uvsimulator->setPC(pc + 1);
					}break;
					default:
		...
		System::Void buttonAddValue_Click(System::Object^ sender, System::EventArgs^ e) {
			if (ValidateInputValue(inputValue->Text)) {
				int pcValue = uvsimulator->getPC();
				int instruction = std::stoi(uvsimulator->getMemory(pcValue));
				int operand = instruction % 100;
				int value;
				if (Int32::TryParse(inputValue->Text, value)) {
;					
					std::string strValue = marshal_as<std::string>(value.ToString());
					if (value >= 0) {
						strValue = "+" + strValue;
					}
					else {}
					uvsimulator->setMemory(operand, strValue);
					PopulateMemoryGrid();
		...
		{
			std:string filePath = marshal_as<std::string>(fileText->Text);
			if (filePath == "") {
		...
		std:string filePath = marshal_as<std::string>(fileText->Text);
			if (filePath == "") {
				...
				LPCWSTR editorPath = L"notepad.exe";
				// Use ShellExecute to open the file
				HINSTANCE result = ShellExecute(NULL, NULL, editorPath, filePath, NULL, SW_SHOWNORMAL);

				// Check if the file was opened successfully
				if ((intptr_t)result <= 32) {
					console->Text = "An error occurred when opening the file.";
				}
			}
		}
		//Currently non-functional, issue saving/loading to bin
		System::Void buttonSave_Click(System::Object^ sender, System::EventArgs^ e)
		{
			...
			// Set properties for the file dialog
			saveFileDialog1->Filter = "Text Files|*.txt|All Files|*.*"; // Specify the file types to be displayed
			saveFileDialog1->Title = "Save As"; // Set the dialog title
			...
			if (saveFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
				...
				uvsimulator->saveFile(selectedFilePath);

				// Notify the user that the file has been created
				System::Windows::Forms::MessageBox::Show("File created successfully", "Success");
			}
		}
